...
de_container_group = L.featureGroup();
de_events_group = L.featureGroup();
de_photo_ops_group = L.featureGroup();
de_feats_group = L.featureGroup();
...
function createDEFeatsMarkers(featsArr) {
    for(var i = 0; i < featsArr.length; i++) {
        window["de_feat" + i + "_marker"] = L.marker([featsArr[i][0], featsArr[i][1]], {icon:window[featsArr[i][2] +"_icon"]}).addTo(de_feats_group);
...
de_container_group.addTo(map);
de_events_group.addTo(map);
de_feats_group.addTo(map);
de_collectibles_group.addTo(map);
de_photo_ops_group.addTo(map);