...
function createVGFeatsMarkers(featsArr) {
    for(var i = 0; i < featsArr.length; i++) {
        window["vg_feat" + i + "_marker"] = L.marker([featsArr[i][0], featsArr[i][1]], {icon:window[featsArr[i][2] +"_icon"]});
...
vg_container_group.push(vg_container_marker);

for(var i = 1; i <= 8; i++) {
    if(i < 10) {
        vg_events_group.push(window["vg_event0" + i + "_marker"]);
    }
    else {
        vg_events_group.push(window["vg_event" + i + "_marker"]);
    }
}

for(var i = 0; i <= 3; i++) {
    vg_feats_group.push(window["vg_feat" + i + "_marker"]);
}

for(var i = 1; i <= 6; i++) {
    if(i < 10) {
        vg_photo_ops_group.push(window["vg_photo_ops0" + i + "_marker"]);
    }
    else {
        vg_photo_ops_group.push(window["vg_photo_ops" + i + "_marker"]);
    }
}

for(var i = 1; i <= 27; i++) {
    if(i < 10) {
        vg_collectibles_group.push(window["vg_collectible0" + i + "_marker"]);
    }
    else {
        vg_collectibles_group.push(window["vg_collectible" + i + "_marker"]);
    }
}

markers_canvas.addMarkers(vg_container_group);
markers_canvas.addMarkers(vg_events_group);
markers_canvas.addMarkers(vg_collectibles_group);
markers_canvas.addMarkers(vg_photo_ops_group);
markers_canvas.addMarkers(vg_feats_group);