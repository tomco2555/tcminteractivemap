...
function createDE2FeatsMarkers(featsArr) {
    for(var i = 0; i < featsArr.length; i++) {
        window["de2_feat" + i + "_marker"] = L.marker([featsArr[i][0], featsArr[i][1]], {icon:window[featsArr[i][2] +"_icon"]});
...
de2_container_group.push(de2_container_marker);

for(var i = 1; i <= 7; i++) {
    if(i < 10) {
        de2_events_group.push(window["de2_event0" + i + "_marker"]);
    }
    else {
        de2_events_group.push(window["de2_event" + i + "_marker"]);
    }
}

for(var i = 0; i <= 5; i++) {
    de2_feats_group.push(window["de2_feat" + i + "_marker"]);
}

for(var i = 1; i <= 8; i++) {
    if(i < 10) {
        de2_photo_ops_group.push(window["de2_photo_ops0" + i + "_marker"]);
    }
    else {
        de2_photo_ops_group.push(window["de2_photo_ops" + i + "_marker"]);
    }
}

for(var i = 1; i <= 5; i++) {
    if(i < 10) {
        de2_collectibles_group.push(window["de2_collectible0" + i + "_marker"]);
    }
    else {
        de2_collectibles_group.push(window["de2_collectible" + i + "_marker"]);
    }
}

markers_canvas.addMarkers(de2_container_group);
markers_canvas.addMarkers(de2_events_group);
markers_canvas.addMarkers(de2_collectibles_group);
markers_canvas.addMarkers(de2_photo_ops_group);
markers_canvas.addMarkers(de2_feats_group);