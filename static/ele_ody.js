...
eo_container_group = L.featureGroup();
eo_events_group = L.featureGroup();
eo_photo_ops_group = L.featureGroup();
eo_feats_group = L.featureGroup();
...
function createEOFeatsMarkers(featsArr) {
    for(var i = 0; i < featsArr.length; i++) {
        window["eo_feat" + i + "_marker"] = L.marker([featsArr[i][0], featsArr[i][1]], {icon:window[featsArr[i][2] +"_icon"]}).addTo(eo_feats_group);
...
eo_container_group.addTo(map);
eo_events_group.addTo(map);
eo_feats_group.addTo(map);
eo_collectibles_group.addTo(map);
eo_photo_ops_group.addTo(map);