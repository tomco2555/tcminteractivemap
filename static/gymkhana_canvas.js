...
function createGymFeatsMarkers(featsArr) {
    for(var i = 0; i < featsArr.length; i++) {
        window["gym_feat" + i + "_marker"] = L.marker([featsArr[i][0], featsArr[i][1]], {icon:window[featsArr[i][2] +"_icon"]});
...
gym_container_group.push(gym_container_marker);

for(var i = 1; i <= 10; i++) {
    if(i < 10) {
        gym_events_group.push(window["gym_event0" + i + "_marker"]);
    }
    else {
        gym_events_group.push(window["gym_event" + i + "_marker"]);
    }
}

for(var i = 0; i <= 5; i++) {
    gym_feats_group.push(window["gym_feat" + i + "_marker"]);
}

for(var i = 1; i <= 5; i++) {
    if(i < 10) {
        gym_photo_ops_group.push(window["gym_photo_ops0" + i + "_marker"]);
    }
    else {
        gym_photo_ops_group.push(window["gym_photo_ops" + i + "_marker"]);
    }
}

for(var i = 1; i <= 7; i++) {
    if(i < 10) {
        gym_collectibles_group.push(window["gym_collectible0" + i + "_marker"]);
    }
    else {
        gym_collectibles_group.push(window["gym_collectible" + i + "_marker"]);
    }
}

markers_canvas.addMarkers(gym_container_group);
markers_canvas.addMarkers(gym_events_group);
markers_canvas.addMarkers(gym_collectibles_group);
markers_canvas.addMarkers(gym_photo_ops_group);
markers_canvas.addMarkers(gym_feats_group);